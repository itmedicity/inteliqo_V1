...
const DoctorRegistrationEdit = () => {
  ...
  const submitemployeerecord = useCallback(() => {
    ...
    if (contractflag === 0 && oldContract_Status === 1) {
      ...
      if (isBefore(new Date(cont_date), new Date()) && cont_date !== '2000-01-31') {
        infoNofity(
          'Employee Contract Date Already Exceeded, You Can Edit This Employee Through Contract Renewal Process!',
        )
      } else {
      ...
      if (isBefore(new Date(probdate), new Date()) && probdate !== '2000-01-31') {
        infoNofity(
          'Employee Probation date already Exceeded!!You Can Edit This Employee Through Company Information!',
        )
      } else {
  ...
  return (
    <>
      <SessionCheck />
      <ToastContainer />
      <CustomLayout title="Doctor Register Edit" displayClose={true}>
          ...
          >
            ...
            <Box sx={{ flex: 1, mt: 0.5, px: 0.5 }}>
              <JoyDoctorTypeSelect
                value={doct}
                setValue={setDoct}
                disabled={doctortype === true ? false : true}
              />
            </Box>
            <Box sx={{ flex: 1, mt: 0.5, px: 0.3 }}>
