...
const DoctorRegistrationTable = () => {
    ...
    }, [dept, deptSect, branch])

    // Employee Record List
    const getEmployeeList = useCallback(async (e) => {
        e.preventDefault()

        if (dept !== 0 && deptSect !== 0 && branch !== 0) {
            const result = await axioslogin.post('/empmast/docyors/list', postData)
            const { success, data } = result.data
            if (success === 1 && data?.length !== 0) {
                setTableData(data)
            } else {
                setTableData([])
                warningNofity("There Is No Doctors Under This Department!")
            }
        } else {
            warningNofity("Choose All Option")
        }
    }, [branch, dept, deptSect, postData])
...
export default memo(DoctorRegistrationTable) 