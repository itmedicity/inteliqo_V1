...
const CpCredential = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (creddoc === true) {
                const result = await axioslogin.post('/PersonalChecklist/CPapprovalData ', personaldata)
                ...
                const result = await axioslogin.post('/PersonalChecklist/CPapprovalNurseData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
                ...
                const result = await axioslogin.post('/PersonalChecklist/CPapprovalParaData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
                ...
                const result = await axioslogin.post('/PersonalChecklist/CPapprovalotheraData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
    ...
    return (
        <Box>
            <ToastContainer />
            <CustomLayout title="CP Approval Credential Privilege" displayClose={true}>
                <Box
...
export default memo(CpCredential)
