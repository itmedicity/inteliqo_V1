...
const CredentialParaModal = ({ isModalOpen, setIsModalOpen, Empdata, count, SetCount }) => {
    ...
    const [FormDatamain, setFormDatamain] = useState({
        ...
        emId: em_id,
        CpRemark: "",
        Cpdatesaved: moment(new Date()).format('YYYY-MM-DD'),
    })
    const { CpRemark, emId, Cpdatesaved } = FormDatamain
    ...
    const personaldata = useMemo(() => {
        return {
            ...
            em_id: emId,
            MSRemark: CpRemark,
            MSdatesaved: Cpdatesaved
        }
    }, [Employee, em_id, CpRemark, Cpdatesaved])
    const handleOnClick = useCallback(async (event) => {
        ...
        if (CpRemark === '') {
            ...
            const result = await axioslogin.post('/PersonalChecklist/CredentialParaCPApproval', personaldata)
...
export default memo(CredentialParaModal)