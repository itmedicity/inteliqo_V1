...
const MdCredential = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (creddoc === true) {
                const result = await axioslogin.post('/PersonalChecklist/MDapprovalData ', personaldata)
                ...
                const result = await axioslogin.post('/PersonalChecklist/MDapprovalNurseData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
                ...
                const result = await axioslogin.post('/PersonalChecklist/MDapprovalParaData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
                ...
                const result = await axioslogin.post('/PersonalChecklist/MDapprovalotheraData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
    ...
    return (
        <Box>
            <ToastContainer />
            <CustomLayout title="MD Approval Credential Privilege" displayClose={true}>
                <Box
...
export default memo(MdCredential)