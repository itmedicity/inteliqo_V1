...
const CredentialOtherModal = ({ isModalOpen, setIsModalOpen, Empdata, count, SetCount }) => {
    ...
    const [FormDatamain, setFormDatamain] = useState({
        ...
        emId: em_id,
        MdRemark: "",
        Mddatesaved: moment(new Date()).format('YYYY-MM-DD'),
    })
    const { MdRemark, emId, Mddatesaved } = FormDatamain
    ...
    const personaldata = useMemo(() => {
        return {
            ...
            em_id: emId,
            MSRemark: MdRemark,
            MSdatesaved: Mddatesaved
        }
    }, [Employee, em_id, MdRemark, Mddatesaved])
    const handleOnClick = useCallback(async (event) => {
        ...
        if (MdRemark === '') {
            ...
            const result = await axioslogin.post('/PersonalChecklist/CredentialOtherMDApproval', personaldata)
    ...
    }, [MdRemark, SetCount, count, personaldata])
    return (
        <Box>
            <Modal
...
export default CredentialOtherModal