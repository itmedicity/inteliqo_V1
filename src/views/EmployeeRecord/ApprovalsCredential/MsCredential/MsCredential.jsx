...
const MsCredential = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (creddoc === true) {
                const result = await axioslogin.post('/PersonalChecklist/MSapprovalData ', personaldata)
                ...
                const result = await axioslogin.post('/PersonalChecklist/MSapprovalNurseData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
                ...
                const result = await axioslogin.post('/PersonalChecklist/MSapprovalParaData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
                ...
                const result = await axioslogin.post('/PersonalChecklist/MSapprovalotheraData ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setTableData(data)
                    SetCount(0)
    ...
    return (
        <Box>
            <ToastContainer />
            <CustomLayout title="MS Approval Credential Privilege" displayClose={true}>
                <Box
...
export default memo(MsCredential)