...
const CredentialParaModal = ({ isModalOpen, setIsModalOpen, Empdata, count, SetCount }) => {
    ...
    const [FormDatamain, setFormDatamain] = useState({
        ...
        emId: em_id,
        MSRemark: "",
        MSdatesaved: moment(new Date()).format('YYYY-MM-DD'),
    })
    const { MSRemark, emId, MSdatesaved } = FormDatamain
    ...
    const personaldata = useMemo(() => {
        return {
            ...
            em_id: emId,
            MSRemark: MSRemark,
            MSdatesaved: MSdatesaved
        }
    }, [Employee, em_id, MSRemark, MSdatesaved])
    const handleOnClick = useCallback(async (event) => {
        ...
        if (MSRemark === '') {
            ...
            const result = await axioslogin.post('/PersonalChecklist/CredentialParaMSApproval', personaldata)
