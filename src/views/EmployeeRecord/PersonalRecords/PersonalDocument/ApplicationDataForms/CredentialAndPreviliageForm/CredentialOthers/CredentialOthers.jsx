...
const CredentialOtherReg = lazy(() => import('./CredentialOtherReg'))


const CredentialOthers = ({ Empdata }) => {
    ...
    const handleOnClick = useCallback(async (event) => {
        ...
        if (HrdNo === 0) {
            ...
            const result = await axioslogin.post('/PersonalChecklist/CredentialOther', postdata)
    ...
    useEffect(() => {
        if (Employee?.length !== 0) {
            const getCommonSettings = async () => {
                const result = await axioslogin.post('/PersonalChecklist/credentialotherTraining ', personaldata)
                const { success, data } = result.data
                if (success === 1 && data?.length > 0) {
                    setUpdateFlag(1)
                    setCredentialTraining(data)
                }
                const resultcerti = await axioslogin.post('/PersonalChecklist/credentialotherCertificate ', personaldata)
                const { successcert, datacert } = resultcerti.data
                if (successcert === 1 && datacert?.length > 0) {

                    setCredentialcert(datacert)
                }
                const resultdetails = await axioslogin.post('/PersonalChecklist/credentialotherdetails ', personaldata)
                ...
                const resultdata = await axioslogin.post('/PersonalChecklist/credentialotherprivilageData ', personaldata)
    ...
    return (
        <Box sx={{ height: window.innerHeight - 220, overflowX: "auto", '::-webkit-scrollbar': { display: "none" }, p: 1, mt: 2 }}>
            ...
            <CredentialOtherReg Employee={Employee} ExpData={ExpData} setExpData={setExpData} HrdNo={HrdNo} setHrdNo={setHrdNo} FormDatamain={FormDatamain} setFormDatamain={setFormDatamain}
                CertificateData={CertificateData} setCertificateData={setCertificateData} Verificationdata={Verificationdata} setVerificationdata={setVerificationdata} Operating={Operating}
                setOperating={setOperating} Credentialcert={Credentialcert} privilageData={privilageData} Training={Training} ApprovalData={ApprovalData} />
...
export default memo(CredentialOthers)