...
const JobSummaryEdit = ({ jobview, selectDesignationName, selectedDeptName, selectDesignation, selectedDept }) => {

    const { updateDesignation, updateSelected, } = useContext(PayrolMasterContext)
    ...
    useEffect(() => {
        const getjobsummary = async () => {
            if (selectDesignation !== 0 && selectedDept !== 0) {
                const result = await axioslogin.post('/jobsummary/getjob/Summary', postData)
                const { success, data } = result.data;
                if (success === 1) {
                    const { equipment_used, objective, reporting_dept, reporting_designation, scope,
                        summary_slno, work_place, working_hour } = data[0]

                    const formdata = {
                        objective: objective,
                        equipment: equipment_used,
                        reporting: reporting_dept,
                        reportDesig: reporting_designation,
    ...
    const sumbitJobSummary = async (e) => {
        ...
        if (jobview === 0) {
            ...
            const result = await axioslogin.post('/jobsummary', postEditData)
