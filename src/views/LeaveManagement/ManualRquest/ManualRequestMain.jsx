...
const ManualRequestMain = () => {
      ...
      if (success === 1 && dta?.plan === differenceCountFromToDate) {
        ...
        if (success === 1) {
          const arr = punchMasteData?.map((val) => {
            const a = modifiedTable?.find(
              (e) => format(new Date(e.date), 'yyyy-MM-dd') === val.duty_day,
            )
            let shiftIn = `${format(new Date(val.duty_day), 'yyyy-MM-dd')} ${format(
              new Date(val.shift_in),
              'HH:mm',
            )}`
            let shiftOut =
              val.shft_cross_day === 0
                ? `${format(new Date(val.duty_day), 'yyyy-MM-dd')} ${format(
                    new Date(val.shift_out),
                    'HH:mm',
                  )}`
                : `${format(addDays(new Date(val.duty_day), 1), 'yyyy-MM-dd')} ${format(
                    new Date(val.shift_out),
                    'HH:mm',
                  )}`
            return {
    ...
    if (remrk === '') {
      ...
      if (success === 0 || success === 1) {
        const lastUpdateDate =
          data?.length === 0
            ? format(startOfMonth(new Date(fromDate)), 'yyyy-MM-dd')
      ...
      cellRenderer: (params) => (
        <>
          <Tooltip title="Delete" followCursor placement="top" arrow>
            <IconButton sx={{ paddingY: 0.5 }} onClick={() => deleteRequest(params)}>
    ...
    },
    {
      ...
      cellRenderer: (params) => {
        if (params.data.filename === null) {
          ...
          return (
            <Tooltip title="View File" followCursor placement="top" arrow>
              <IconButton sx={{ paddingY: 0.5 }} onClick={() => getFileData(params)}>
  ...
  const getFileData = async (params) => {
    const { filename } = params.data
     try {
      const result = await axioslogin.get(`/upload/getmanualReq/${filename}`, {
        responseType: 'blob',
      })
      ...
      setFileUrls(images[0]?.url)
      setFileOpen(true)
    } catch (error) {
      console.error('Error fetching or processing profile image:', error)
      // Optionally show error message to user or handle fallback
    }
  }
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
          ...
          <Box sx={{}}>
            <Paper
              variant="outlined"
              sx={{
                maxHeight: (screenInnerHeight * 40) / 100,
                m: 0.3,
                overflow: 'auto',
              }}
            >
                    ...
                    table?.map((val, idx) => (
                      <tr key={idx} style={{ p: 0, m: 0 }}>
                        <td style={{ textAlign: 'center' }}>
                          <Box
                            sx={{ display: 'flex', justifyContent: 'space-between', px: 3, py: 0 }}
                          >
                            <Box>
                              <Typography
                                level="title-md"
          ...
          <Box sx={{ display: 'flex', flex: 1, p: 1, flexWrap: 'wrap', gap: 0.5 }}>
            ...
            <Box>
              <CssVarsProvider>
                <Tooltip title="Upload Documents" variant="outlined" placement="top">
                  <Button variant="outlined" component="label" size="lg" color="danger">
                    <UploadFileIcon />
                    <input
                      //hidden
                      id="file-input"
            ...
            <Box sx={{ display: 'flex' }}>
              <CssVarsProvider>
                <Tooltip
                  title="Click Here to Save  Request"
                  followCursor
                  placement="top"
                  arrow
                  variant="outlined"
