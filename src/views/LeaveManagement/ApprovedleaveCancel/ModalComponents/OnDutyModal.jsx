...
const OnDutyModal = ({ open, setOpen, ondutyData, setCount }) => {
    ...
    const handleApproverequest = useCallback(async () => {
        ...
        const monthStartDate = format(startOfMonth(new Date(ondutyData?.on_duty_date)), 'yyyy-MM-dd')
        const postData = {
            month: monthStartDate,
            section: ondutyData?.dept_sect_id
        }
        const checkPunchMarkingHr = await axioslogin.post("/attendCal/checkPunchMarkingHR/", postData);
        const { success, data } = checkPunchMarkingHr.data
        if (success === 0 || success === 1) {
            const lastUpdateDate = data?.length === 0 ? format(startOfMonth(new Date(ondutyData?.on_duty_date)), 'yyyy-MM-dd') : format(new Date(data[0]?.last_update_date), 'yyyy-MM-dd')
            const lastDay_month = format(lastDayOfMonth(new Date(ondutyData?.on_duty_date)), 'yyyy-MM-dd')
            if ((lastUpdateDate === lastDay_month) || (lastUpdateDate > lastDay_month)) {
                warningNofity("Punch Marking Monthly Process Done !! Can't Cancel On Duty Request  ")
                setOpen(false)
            } else {
                const punchmast = {
                    empno: ondutyData?.em_no,
                    dutyday: ondutyData?.on_duty_date
                }
                ...
                if (success === 1) {
                    ...
                    const postData = {
                        ...
                        slno: ondutyData?.onduty_slno,
                        punch_slno: punch_slno,

                    }
                    const result = await axioslogin.post('/CommonReqst/cancel/approvedOnDuty', postData)
                ...
                <Box sx={{
                    ...
                    <Box sx={{ flex: 1, width: '100%' }} >
                        <Typography sx={{ fontWeight: 600 }} >
                            One Duty Approved Request Cancel
                        </Typography>
                    </Box>
                </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} >
                        <Box sx={{ display: 'flex', }}>
                            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2, }} >
                                <Typography level="title-lg">{ondutyData?.em_name}</Typography>
                                <Typography level="title-lg">Employee No # {ondutyData?.em_no}</Typography>
                            </Box>
                        </Box>
                        <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center' }} >
                            ...
                            <Typography level="body-sm" color="primary" variant="plain" noWrap sx={{ mx: 0.5 }} >{ondutyData?.sect_name}</Typography>
                        </Box>
                    </Box>
