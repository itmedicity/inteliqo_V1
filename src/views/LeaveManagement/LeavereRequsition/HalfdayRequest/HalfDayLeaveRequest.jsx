...
import {
  getCaualLeaveDetl,
  getCommonSettings,
  getEmployeeInformationLimited,
  getInchargeHodAuthorization,
  getLeaveReqApprovalLevel,
...
const HalfDayLeaveRequest = ({ setRequestType, setCount }) => {
  ...
  const {
    hod: loginHod,
    incharge: loginIncharge,
    em_no: loginEmno,
    groupmenu,
  } = empInformationFromRedux
    ...
    async (date) => {
      ...
      if (success === 1) {
        ...
        const postDataForpunchMaster = {
          date1:
            shft_cross_day === 0
              ? format(addHours(new Date(chekOut), comp_hour_count), 'yyyy-MM-dd H:mm:ss')
              : format(
                  addHours(new Date(addDays(new Date(fromDate), 1)), comp_hour_count),
                  'yyyy-MM-dd H:mm:ss',
                ),
          date2:
            shft_cross_day === 0
      ...
      if (lastUpdateDate === lastDay_month || lastUpdateDate > lastDay_month) {
        ...
        if (selectedCL === 0 || reson === '') {
          ...
          if (holiday_leave_request === 1 && holiday === 1) {
            ...
            const first_half_inTime = `${format(new Date(fromDate), 'yyyy-MM-dd')} ${format(
              new Date(first_half_in),
              'HH:mm',
            )}`
            const first_half_outTime = `${format(new Date(fromDate), 'yyyy-MM-dd')} ${format(
              new Date(first_half_out),
              'HH:mm',
            )}`
            const second_half_inTime = `${format(new Date(fromDate), 'yyyy-MM-dd')} ${format(
              new Date(second_half_in),
              'HH:mm',
            )}`
            const second_half_outTime = `${format(new Date(fromDate), 'yyyy-MM-dd')} ${format(
              new Date(second_half_out),
              'HH:mm',
            )}`
            ...
            const result = await axioslogin.post(
              '/LeaveRequest/inserthalfdayreque',
              halfdaysavedata,
            )
  ...
  return (
    <Box sx={{ mb: 0.5 }}>
      ...
      <Paper variant="outlined" sx={{ mt: 0.5 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-around', px: { xl: 0, lg: 20 } }}>
            <Box sx={{ flex: 1, px: 0.3 }}>
              ...
              >
                ...
                {casualLve?.map((e) => (
                  <Option key={e.slno} value={e.slno}>
                    {e.month}
                  </Option>
                ))}
              </Select>
        ...
        <Paper variant="outlined" sx={{ display: 'none' }}></Paper>
        <Box sx={{ p: 1 }}>
          <Textarea
            color="primary"
            minRows={2}
            defaultValue=""
            placeholder="Halfday Leave Request Reason ..."
            size="sm"
            variant="outlined"
