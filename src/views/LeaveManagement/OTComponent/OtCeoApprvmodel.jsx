...
const OtCeoApprvmodel = ({ open, handleClose, rowData, inchid, count, setCount }) => {
    ...
    const [ot_ceo_remark, setOt_ceo_remark] = useState('')
    ...
    }, [over_time, em_no, array, ot_coff_type, check_in, check_out])
    ...
    const patchData = {
        ot_ceo_status: apprv === true ? 1 : reject === true ? 2 : 0,
    ...
    const submitdata = async () => {
        ...
        if (success === 2) {
            if (apprv === true && reject === false && flag === 3) {
                succesNofity("CEO Approved & Converted to Over Time");
                ...
                if (success === 1) {
                    succesNofity("Ot Approval CEO Rejected ");
                } else {
                    infoNofity(message)
                }
            } else if (apprv === true && reject === false && flag === 1) {
                succesNofity("CEO Approved & Compensatory Off Added ");
            } else if (apprv === false && reject === true && flag === 1) {
                succesNofity("Compensatory Off CEO rejected ");
            }
            setCount(count + 1)
            setFlag(0)
            handleClose()
                    ...
                    <Paper square variant="outlined" sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', }} >
                        <Box sx={{ width: "100%", overflow: 'auto', '::-webkit-scrollbar': { display: "none" } }} >
                            <Box sx={{ display: "flex", flex: 1, px: 0.5, justifyContent: "left", fontWeight: 500 }}  >
                                <CssVarsProvider>
                                    <Typography fontSize="xl" level="body1">Over Time CEO Approval </Typography>
                                </CssVarsProvider>
                            </Box>
                            <Paper square variant="outlined" sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', }} >
                                ...
                                <Box sx={{ display: 'flex', flex: 6 }} >
                                    <CssVarsProvider>
                                        <Textarea
                                            ...
                                            minRows={1}
                                            maxRows={3}
                                            onChange={(e) => setOt_ceo_remark(e.target.value)}
                                            sx={{ flex: 1 }}
                                        />
                                    </CssVarsProvider>
                                </Box>
...
export default memo(OtCeoApprvmodel)