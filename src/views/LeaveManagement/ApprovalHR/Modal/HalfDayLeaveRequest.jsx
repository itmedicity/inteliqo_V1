...
const HalfDayLeaveRequest = ({ open, setOpen, data, setCount }) => {
  ...
  const handleApproverequest = useCallback(async () => {
    ...
    if (reason === '') {
      ...
      const checkPunchMarkingHr = await axioslogin.post(
        ...
        postDataForAttendaceMark,
      )
      const { success, data } = checkPunchMarkingHr.data
      if (success === 0 || success === 1) {
        const lastUpdateDate =
          data?.length === 0
            ? format(startOfMonth(new Date(leavedate)), 'yyyy-MM-dd')
        ...
        if (lastUpdateDate === lastDay_month || lastUpdateDate > lastDay_month) {
          warningNofity(
            "Punch Marking Monthly Process Done !! Can't Approve Halfday Leave Request!!  ",
          )
          setOpenBkDrop(false)
          setOpen(false)
        } else {
          const postData = {
            ...
            preToDate:
              crossDayStat === 0
                ? format(addHours(new Date(leavedate), 8), 'yyyy-MM-dd 23:59:59')
                : format(
                    addHours(new Date(addDays(new Date(leavedate), 1)), 8),
                    'yyyy-MM-dd 23:59:59',
                  ),
            empList: emno,
          }
          ...
          if (su === 1 && result_data?.length !== 0) {
            ...
            const punch_master_data = await axioslogin.post(
              '/attendCal/attendanceshiftdetl/',
              punchmastData,
            ) //GET PUNCH MASTER DATA
            const { success, data } = punch_master_data.data
            if (success === 1) {
              let shiftIn = `${format(new Date(leavedate), 'yyyy-MM-dd')} ${format(
                new Date(crossDay?.checkInTime),
                'HH:mm:ss',
              )}`
              let shiftOut =
                crossDayStat === 0
                  ? `${format(new Date(leavedate), 'yyyy-MM-dd')} ${format(
                      new Date(crossDay?.checkOutTime),
                      'HH:mm:ss',
                    )}`
                  : `${format(addDays(new Date(leavedate), 1), 'yyyy-MM-dd')} ${format(
                      new Date(crossDay?.checkOutTime),
                      'HH:mm:ss',
                    )}`
                  ...
                  punchMasterMappedData?.map(async (val) => {
                    ...
                    const punch_In =
                      halfday_status === 1
                        ...
                        : val.punch_in === null
                        ? null
                        : new Date(val.punch_in)
                    const punch_out =
                      halfday_status === 2
                        ? new Date(shiftOut)
                        : val.punch_out === null
                        ? null
                        : new Date(val.punch_out)
                    ...
                    const getLateInTime = await getLateInTimeIntervel(
                      punch_In,
                      shift_in,
                      punch_out,
                      shift_out,
                    ...
                    return {
                      ...
                      punch_in:
                        halfday_status === 1
                          ? format(new Date(punch_In), 'yyyy-MM-dd HH:mm:ss')
                          : val.punch_in,
                      punch_out:
                        halfday_status === 2
                          ? format(new Date(punch_out), 'yyyy-MM-dd HH:mm:ss')
                          : val.punch_out,
                      hrs_worked:
                        val.shift_id === week_off_day ||
                        val.shift_id === noff ||
                        val.shift_id === notapplicable_shift ||
                        val.shift_id === default_shift
                          ? 0
                          : getLateInTime?.hrsWorked,
                      late_in:
                        val.shift_id === week_off_day ||
                        val.shift_id === noff ||
                        val.shift_id === notapplicable_shift ||
  ...
  const handleRegectRequest = useCallback(async () => {
    const result = await axioslogin.patch(
      `/LeaveRequestApproval/HalfDayReqRejectHr`,
      LeaveRejectdata,
    )
