...
import { getAllHodApprvl } from 'src/redux/actions/OTAction'
...
import OtHodApprvlModel from '../OTComponent/OtHodApprvlModel'
...
const OtHodApproval = () => {
    ...
    useEffect(() => {
        dispatch(getAllHodApprvl(alldept))
    }, [dispatch, alldept, count])
    ...
    const alldata = useSelector((state) => state?.getOTApprovalData?.approvalHod?.approvalHodList, _.isEqual)
    ...
    const [columnDef] = useState([
        ...
        { headerName: 'Status ', field: 'hod_status', minWidth: 200 },
        {
            headerName: 'Action',
            cellRenderer: params => {
                if (params.data.ot_hod_status === 1) {
                    return <Fragment>
                        <IconButton
                            sx={{ paddingY: 0.5 }} >
    ...
    return (
        <CustomLayout title="OT HOD Approval" displayClose={true} >
            <ToastContainer />
            {
                apprvFlag === 1 ? <OtHodApprvlModel open={apprvOpen} handleClose={handleClose}
            ...
            }
            {
                canclFlag === 1 ? <OtCnclModel open={cancelOpen} handleClose={handleCancelClose}
                    ...
                    apprvlname={"OT Hod Approval Cancel"} /> : null
            }
            <Paper sx={{ width: '100%' }}>
...
export default memo(OtHodApproval) 