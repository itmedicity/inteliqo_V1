...
const OtReqestMainPage = () => {
    ...
    const { hod, incharge, em_id, em_department, em_no, em_name, sect_name, em_dept_section, desg_name } = empData
    ...
    }, [em_department, em_dept_section, em_id, dispatch])
    ...
    const { authorization_incharge, authorization_hod, co_assign } = approvalrights;
    ...
    }, [em_id, em_no, fromDate, selectedShift, punchInTime, punchOutTime, overTime,
        ot_reason, ot_remark, amount, hod, incharge, co_assign, em_dept_section, dutyday,
        authorization_hod, authorization_incharge])
    ...
    const deletOTRequest = async (params) => {
        ...
        if (success === 1) {
            setCount(count - 1)
            const result = await axioslogin.patch('/overtimerequest/inactive/punch', newArry)
    ...
    return (
        <Fragment>
            {
                Flag === 1 ? < InchargeHodPage em_id={em_id} em_department={em_department} /> :
...
export default memo(OtReqestMainPage) 