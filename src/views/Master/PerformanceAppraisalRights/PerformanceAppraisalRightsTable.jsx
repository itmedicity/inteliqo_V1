...
const PerformanceAppraisalRightsTable = ({ count }) => {
    ...
    useEffect(() => {
        const getAPrraisalRights = async () => {
            const result = await axioslogin.get('/performanceappriasalrights/data')
    ...
    const getTableData = (data) => {
        ...
        history.push(`/Home/PerformanceAppraisalRightsEdit/${p_rights_slno}`)
    }

    return (
        <MaterialTable
            title="Performance Appraisal Rights "
            data={value}
            columns={title}
            icons={tableIcons}
            actions={[
...
export default PerformanceAppraisalRightsTable