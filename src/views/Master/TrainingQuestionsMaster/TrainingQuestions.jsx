...
const TrainingQuestions = () => {
  ...
  //written checkbox
  const HandleWrittenStatus = useCallback(
    (e) => {
  ...
  //options checking
  const HandleStatus_a = useCallback(
    (e) => {
      if (e.target.checked === true) {
  ...
  const HandleStatus_b = useCallback(
    (e) => {
      if (e.target.checked === true) {
  ...
  const HandleStatus_c = useCallback(
    (e) => {
      if (e.target.checked === true) {
  ...
  const HandleStatus_d = useCallback(
    (e) => {
      if (e.target.checked === true) {
  ...
  const handleRemoveFileA = useCallback(
    (e) => {
      setShowCloseIconA(false)
      setSelectedFileA(null)
    },
  ...
  const handleRemoveFileB = useCallback(
    (e) => {
      setShowCloseIconB(false)
      setSelectedFileB(null)
    },
  ...
  const handleRemoveFileC = useCallback(
    (e) => {
      setShowCloseIconC(false)
      setSelectedFileC(null)
    },
  ...
  const handleRemoveFileD = useCallback(
    (e) => {
      setShowCloseIconD(false)
      setSelectedFileD(null)
    },
    ...
    const handleUpload = async (insetId) => {
      try {
        ...
        const uploadResult = await axioslogin.post(
          '/trainUploadCheck/uploadtrainingfiles',
          formData,
          {
            headers: {
        ...
        warningNofity('An error occurred.')
  ...
  return (
    <CustomSettingsLayout title="Training Questions Master" displayClose={true}>
      ...
      >
        <Paper
          variant="outlined"
          sx={{ p: 1, width: '100%', display: 'flex', flexDirection: 'column' }}
        >
          <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1 }}>
            <Box sx={{ width: '20%' }}>
              <Typography sx={{ fontFamily: 'sans-serif', fontWeight: '600' }}>Topic</Typography>
              <TrainingTopicsRdx
                topicValue={training_topic}
                getTopic={setTraining_topic}
              ...
              <Box sx={{ display: 'flex', flexDirection: 'row', flex: 1, mt: 2 }}>
                <Tooltip title="Upload file">
                  <IconButton variant="outlined" component="label">
                    <UploadFileIcon
                      style={{ color: '#4682A9', fontSize: 40, border: 1, borderRadius: 10 }}
                    />
                    <input
                      id="file-input"
                      type="file"
                  ...
                  <Box sx={{ mt: 1 }}>
                    <Tooltip title="View file">
                      <IconButton onClick={(e) => ShowFliesA(e)}>
                        <TouchAppSharpIcon sx={{ color: '#3876BF' }} />
                      </IconButton>
                    </Tooltip>
                  </Box>
                ) : null}
