...
const ContractCurrentRunningReport = () => {
    ...
    const getCurrentRunningContract = useCallback((e) => {
        e.preventDefault();
        dispatch({ type: Actiontypes.FETCH_CHANGE_STATE, aggridstate: 0 })
        /** branch wise Current running contract report  */
        const getBranchRunningContract = async (slno) => {
            const result = await axioslogin.post('/ContractReport/branchcontractrunning', slno)
        ...
        /** Department  wise  Current running contract report  */
        const getDeptRunningContract = async (postData) => {
            const result = await axioslogin.post('/ContractReport/deptcontractrunning', postData)
            const { success, data } = result.data;
            if (success === 1) {
                setTableData(data)
            }
        ...
        /** Selected branch, department, dept section  Current running contract report  */
        const getCurrentRunningContract = async (postDataemp) => {
            const result = await axioslogin.post('/ContractReport/currentrunning', postDataemp)
            const { success, data } = result.data;
            if (success === 1) {
                setTableData(data)
            }
        ...
        if (slno !== 0 && deptslno === 0 && sectslno === 0) {
            getBranchRunningContract(slno)
        }
        else if (slno !== 0 && deptslno !== 0 && sectslno === 0) {
            getDeptRunningContract(postData)
        }
        else if (slno !== 0 && deptslno !== 0 && sectslno !== 0) {
            getCurrentRunningContract(postDataemp)
        }
        else {
    ...
    return (
        <Fragment>
            ...
            <CustomReportMain
                ...
                /**  Current running contract report List */
                columnDefMain={columnDefMain}
                onClick={getCurrentRunningContract}
...
export default ContractCurrentRunningReport