...
import InactiveEmployeeSelect from 'src/views/MuiComponents/JoyComponent/InactiveEmployeeSelect'


const InactiveEmployeeAttendnaceReport = () => {
    const [deptName, setDepartmentName] = useState(0)
    const [deptSecName, setDepartSecName] = useState(0)
    const [Empno, setEmpNo] = useState({ em_id: 0, em_name: 'Employee Name', em_no: 0 })
    ...
    const getData = useCallback(async () => {
        ...
        if (deptName !== 0 && fromdate !== '' && todate !== '' && deptSecName !== 0 && Empno?.em_id === 0) {
            ...
            if (success === 0 || success === 1) {
                ...
                if ((lastUpdateDate === lastDay_month) || (lastUpdateDate > lastDay_month)) {
                    ...
                    if (success === 1) {
                        const empids = data?.map((val) => val.em_id)
                        ...
                        if (success === 1) {
                            ...
                            warningNofity("There is no dutyplan data")
                            setTableData([])
                            setOpenBkDrop(false)
                        }
                    } else {
                        warningNofity("There is no employee under this department")
                        setTableData([])
                        setOpenBkDrop(false)
                    }
                    ...
                    if (success === 1 && data?.length > 0) {
                        // const empData = data;
                        const punchdep_data = await axioslogin.post("/AttendenceReport/getInactivePunchDataDptWiseDateWise/", postPunchData);
                        const { su, resultPunch_data } = punchdep_data?.data;

                        if (su === 1 && resultPunch_data?.length > 0) {
                            const punchaMasData = resultPunch_data;
                            const punchResult = await axioslogin.post("/AttendenceReport/getInactivePunchMasterDataDeptWise", postPunchData)
                            ...
                            setOpenBkDrop(false)
                            infoNofity("There Is No Punch Deatils of Selected Departments ")
                            // setEmpArray([])
                        }
                    } else {
                        setOpenBkDrop(false)
        ...
        } else if (deptName !== 0 && fromdate !== '' && todate !== '' && deptSecName !== 0 && Empno?.em_id !== 0) {
            ...
            if (success === 0 || success === 1) {
                ...
                if ((lastUpdateDate === lastDay_month) || (lastUpdateDate > lastDay_month)) {
                    ...
                    dispatch({ type: Actiontypes.FETCH_CHANGE_STATE, aggridstate: 0 })
                    const punch_data = await axioslogin.post("/AttendenceReport/getInactivePunchDataDptWiseDateWise/", postData);
                    const { su, result_data } = punch_data.data;
                    if (su === 1 && result_data?.length > 0) {
                        const punchaData = result_data;
                        const punch_master_data = await axioslogin.post("/AttendenceReport/getInactivePunchMasterDataDeptWise/", postData); //GET PUNCH MASTER DATA
                        ...
                        warningNofity("There Is No Punch Details Against This Employee")
                        setOpenBkDrop(false)
                    }
                }
            } else {
        ...
        cmmn_grace_period, Empno, postPunchData, halfday_time_count, cmmn_late_in, salary_above,
        week_off_day, notapplicable_shift, default_shift, dispatch, noff])
    ...
    return (
        <Box sx={{ display: "flex", flexGrow: 1, width: "100%", }} >
            ...
            <ReportLayout title="Inactive Employee Attendence Report" displayClose={true} data={tableData} >
                <Paper sx={{ display: 'flex', flex: 1, flexDirection: 'column', }}>
                    <Box sx={{ mt: 1, ml: 0.5, display: 'flex', flex: { xs: 4, sm: 4, md: 4, lg: 4, xl: 3, }, flexDirection: 'row', }}>
                        ...
                        <Box sx={{ flex: 1, px: 0.5 }}>
                            ...
                            <InactiveEmployeeSelect deptName={deptName} deptSecName={deptSecName} setEmpNo={setEmpNo} Empno={Empno} />
...
export default memo(InactiveEmployeeAttendnaceReport) 