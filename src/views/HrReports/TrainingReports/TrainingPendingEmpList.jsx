...
const TrainingPendingEmpList = () => {
    ...
    const getData = useCallback(() => {
        ...
        if (dept !== 0 && deptSect !== 0 && topic !== 0) {
            const SelectDatas = async (postdata) => {
                const result = await axioslogin.post(`/TrainingMonthlyReport/getPendingList`, postdata)
    ...
    return (
        <Fragment>
            <ToastContainer />
            <ReportLayout title=" Departmental Training Pending Report" data={datas} displayClose={true} >
                <Paper sx={{ width: '100%' }}>
...
export default memo(TrainingPendingEmpList)
