...
const InactiveDoctorreport = () => {
  ...
  const getemployeedetails = async () => {
    setOpenBkDrop(true)
    if (all === true) {
      const result = await axioslogin.get('/ActiveEmpReport/allinactive/doctorlist')
      ...
      const result = await axioslogin.post('/ActiveEmpReport/allinactive/branchlist', branch_slno)
      const { success, data } = result.data
  ...
  return (
    <ReportLayout title="Inactive Doctors Report" displayClose={true} data={tableData}>
      ...
      <Box sx={{ width: '100%', p: 0.5, display: 'flex', flexDirection: 'column' }}>
        ...
        <Box sx={{ p: 1, mt: 0.5, display: 'flex', flexDirection: 'column', width: '100%' }}>
          <CustomAgGridRptFormatOne tableDataMain={tableData} columnDefMain={columnDefMain} />
        </Box>
      </Box>
    </ReportLayout>
  )
...
export default memo(InactiveDoctorreport)
