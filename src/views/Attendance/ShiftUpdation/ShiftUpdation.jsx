...
import {
  dailyPunchMarkingFunction,
  processShiftPunchMarkingHrFunc,
} from '../PunchMarkingHR/punchMarkingHrFunc'
...
const ShiftUpdation = () => {
  ...
  const { week_off_day, notapplicable_shift, default_shift, noff, doff, second_plicy } =
    commonSetting
  ...
  const shiftPunchMarkingHandleClickFun = useCallback(
    async (e) => {
      e.preventDefault()
          ...
          if (lastUpdateDate === lastDay_month || lastUpdateDate > lastDay_month) {
            ...
            if (success === 1) {
              const tb = planData?.map((e) => {
                ...
                let shiftIn = `${format(new Date(e.duty_day), 'yyyy-MM-dd')} ${format(
                  new Date(e.shift_in),
                  'HH:mm',
                )}`
                let shiftOut =
                  crossDayStat === 0
                    ? `${format(new Date(e.duty_day), 'yyyy-MM-dd')} ${format(
                        new Date(e.shift_out),
                        'HH:mm',
                      )}`
                    : `${format(addDays(new Date(e.duty_day), 1), 'yyyy-MM-dd')} ${format(
                        new Date(e.shift_out),
                        'HH:mm',
                      )}`
                ...
                return {
                  ...
                  em_no: e.em_no,
                  punch_in:
                    e.shift_id === default_shift ||
                    e.shift_id === notapplicable_shift ||
                    e.shift_id === week_off_day ||
                    e.shift_id === noff
                      ? crossDay?.shft_desc
                      : e.punch_in,
                  punch_out:
                  ...
                  hrs_worked:
                    isValid(new Date(e.punch_in)) &&
                    e.punch_in !== null &&
                    isValid(new Date(e.punch_out)) &&
                    e.punch_out !== null
                      ? formatDuration({
                          days: interVal.days,
            ...
            const postData_getPunchData = {
              ...
              preToDate:
                todayStatus === true
                  ? format(addDays(lastDayOfMonth(new Date(value)), 1), 'yyyy-MM-dd 23:59:59')
                  : format(addDays(new Date(value), 1), 'yyyy-MM-dd 23:59:59'),
            ...
            const punch_data = await axioslogin.post(
              '/attendCal/getPunchDataEmCodeWiseDateWise/',
            ...
            if (su === 1) {
              ...
              if (second_plicy === 1) {
                //checking second policy is running
                ...
                if (status === 1) {
                  const tb = punchMastData?.map((e) => {
                    const crossDay = shiftInformation?.find(
                      (shft) => shft?.shft_slno === e?.shift_id,
                    )
                    const crossDayStat = crossDay?.shft_cross_day ?? 0
                ...
                <TableHead>
                  <TableRow sx={{ color: '#003A75' }} hover>
                    <TableCell
                      ...
                      align="center"
                      rowSpan={2}
                      sx={{ color: '#003A75', fontWeight: 550 }}
                    >
                      #
                    </TableCell>
                    <TableCell
