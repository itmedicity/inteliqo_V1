...
const SelectionModal = lazy(() => import('./SelectionModal'))
...
const Selection = () => {
                        ...
                        const updatedNewEmp = newEmp.map((item) => {
                            ...
                            const updatedApslno = item.apslno.map((apslnoItem) => {
                                ...
                                return foundStatus ? {
                                    ...
                                    totalmark: (foundStatus.Hr_Interview_Mark + foundStatus.total_Incharge_inter_mark + foundStatus.Hod_interview_mark + foundStatus.Ms_Interview_Mark + foundStatus.Dms_Interview_Mark +
                                        foundStatus.Operation_Interview_Mark + foundStatus.Ceo_Interview_Mark), applicationform_id: foundStatus.applicationForm_id, selectedStatus: foundStatus.selected_status
                                } : apslnoItem;
                            });
    ...
    // const handleonclick = useCallback(async (idx, item) => {
    //     setIsModalOpen(true)
    ...
    return (
        <Box sx={{ display: 'flex', flex: 1, flexDirection: 'column', height: window.innerHeight - 100, overflowX: "auto" }}>
            <DasboardCustomLayout title={"Candidate Selection"} displayClose={true} setClose={toRedirectToHome}>
                            ...
                            <Table sx={{ p: 0, width: '100%' }}>
                                <TableHead >
                                    <TableRow>
                                        ...
                                        <TableCell colSpan={2}><Typography>Desgnation</Typography></TableCell>
                                        <TableCell colSpan={2}><Typography>Required no</Typography></TableCell>
                                        <TableCell colSpan={3}><Typography>Department</Typography></TableCell>
                                    ...
                                    {Interviewlist?.map((val, index) => {
                                        ...
                                        return (
                                            <Fragment key={val.desg_id}>
                                                <TableRow>
                                                    <TableCell colSpan={1} size='small' padding='none' sx={{ ml: 1, textAlign: "center" }}>
                                                    ...
                                                    <TableCell colSpan={2} size='small'  >{val.desg_name}</TableCell>
                                                    <TableCell colSpan={2} size='small' >{val.manpower_required_no}</TableCell>
                                                    <TableCell colSpan={3} size='small' >{val.dept_name}</TableCell>
                                                ...
                                                {isRowOpen && (
                                                    <TableRow padding='none' sx={{ backgroundColor: "#F8F6F4" }}>
                                                        ...
                                                        <TableCell size='small' ><Typography sx={{ color: "#808B97", }}>Rank</Typography></TableCell>
                                                        <TableCell size='small' ><Typography sx={{ color: "#808B97", }}>Total Mark</Typography></TableCell>
                                                        <TableCell size='small'><Typography sx={{ color: "#808B97", }}>Candidate Selection</Typography>   </TableCell>
                                                        <TableCell size='small'></TableCell>
                                                    </TableRow>
                                                )}

                                                {isRowOpen && matchingMainData && (
                                                    <>
                                                        {matchingMainData.apslno
                                                            ...
                                                            .map((item, idx) => (
                                                                ...
                                                                < TableRow key={idx} sx={{ backgroundColor: "#F8F6F4" }}>
                                                                    ...
                                                                    <TableCell size='small'>{item?.totalmark}</TableCell>
