...
const ModalHr = ({ setIsModalOpen, isModalOpen, selectedRowData, count, setcount }) => {
    ...
    const handleCloseModal = useCallback(async (event) => {
        ...
        if (remark === '') {
            ...
            const result = await axioslogin.post('/Manpower/updateHrApproval', rejectpostdata)
    ...
    const handleOnClick = useCallback(async (event) => {
        ...
        if (remark === '') {
            ...
            const result = await axioslogin.post('/Manpower/updateHrApproval', postdata)
            const { success, message } = result.data
            if (success === 2) {
                setcount(count + 1)
                succesNofity(message)
    ...
    }, [postdata, count, remark, setIsModalOpen, setcount])
    return (
        <Box>
