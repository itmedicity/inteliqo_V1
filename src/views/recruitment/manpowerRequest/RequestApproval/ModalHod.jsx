...
const ModalHod = ({ setIsModalOpen, isModalOpen, selectedRowData, setcount, count }) => {
    ...
    const handleCloseModal = useCallback(async (event) => {
        ...
        if (remark === '') {
            ...
            const result = await axioslogin.post('/Manpower/updateHodApproval', rejectpostdata)
    ...
    const handleOnClick = useCallback(async (event) => {
        ...
        if (remark === '') {
            ...
            const result = await axioslogin.post('/Manpower/updateHodApproval', postdata)
            const { success, message } = result.data
            if (success === 2) {
                setcount(count + 1)
                succesNofity(message)
                    ...
                    <Box sx={{ p: 2, border: 1, borderColor: "#6B728E" }}>
                        ...
                        <Box sx={{ mt: 2, display: "flex" }}>
                            ...
                            <Box sx={{ width: "50%" }}>
                                ...
                                <Typography
                                    sx={{ color: '#78C1F3' }}>
                                    {selectedRowData?.apprentice_status === 1 ? "Apprentice"
                                ...
                                <Typography sx={{ color: '#78C1F3' }}>
                                    {selectedRowData?.addition_status === 1 ? "Addition"
                                        : selectedRowData?.new_position_statu === 1 ? "New Position"
                                            : selectedRowData?.replacement_status === 1 ? 'Replacement' : "nil"
                                    }</Typography>
                                {selectedRowData?.replacement_status === 1 ?
                                    <Typography sx={{ textTransform: "capitalize", color: '#78C1F3' }}>
                                        {selectedRowData?.em_name || 'N/A'}</Typography> : null}
