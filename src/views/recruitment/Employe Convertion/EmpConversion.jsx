...
const ConvertionModal = lazy(() => import('./ConvertionModal'))
...
const EmpConversion = () => {
    ...
    const handleonclick = useCallback(async (idx, item) => {
        ...
        if (success === 1) {
            ...
            if (success1 === 1) {
                const { assigned_join_date } = data1[0]
                if (data1 && data1.length > 0) {
                    const arr = data.map((val) => {
                        return {
                            ...val, desgid: item?.desg_id, dateofjoing: assigned_join_date
                        }
                    })
    ...
    return (
        <Box sx={{ display: 'flex', flex: 1, flexDirection: 'column', height: window.innerHeight - 100, overflowX: "auto" }}>
            <DasboardCustomLayout title={"Employee Conversion"} displayClose={true} setClose={toRedirectToHome}>
                                            ...
                                            <Fragment key={val.desg_id}>
                                                ...
                                                {isRowOpen && (
                                                    <TableRow padding='none' sx={{ backgroundColor: "#F8F6F4" }}>
                                                        ...
                                                        <TableCell size='small' ><Typography sx={{ color: "#808B97", }}>Employee conversion</Typography></TableCell>
                                                        ...
                                                        return (
                                                            <TableRow key={idx} sx={{ backgroundColor: "#F8F6F4" }}>
                                                                ...
                                                                <TableCell size='small' sx={{ cursor: 'pointer' }} >
                                                                    <PreviewIcon onClick={(e) => handleonclick(e, item)} />
                                                                </TableCell>
                                                                {/* {item?.offerletter_status === 1 ?
                                                                    <TableCell size='small'>Offer Letter send</TableCell> :
...
export default memo(EmpConversion)