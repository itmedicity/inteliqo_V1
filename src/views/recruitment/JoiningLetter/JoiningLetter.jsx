...
const JoiningLettermodal = lazy(() => import('./JoiningLettermodal'))
...
const JoiningLetter = () => {
                        ...
                        const updatedNewEmp = newEmp.map((item) => {
                            ...
                            const updatedApslno = item.apslno.map((apslnoItem) => {
                                ...
                                return foundStatus ? {
                                    ...
                                    offerletter_status: foundStatus.offer_letterstatus, joining_status: foundStatus.joining_status, join_ltr_cancel_status: foundStatus.join_ltr_cancel_status
                                } : apslnoItem;
                            });
        ...
        if (success === 1) {
            ...
            if (success1 === 1) {
                ...
                if (data1 && data1.length > 0) {
                    ...
                    const result = await axioslogin.post('/Applicationform/Joinpdfdata', checkdata)
                    ...
                    if (success === 1) {
                        ...
                        const result = await axioslogin.post('/Applicationform/insertjoining', AppointmentData)
                        const { success } = result.data
                        if (success === 1) {
                            setIsModalOpen(true)
                            setpersonaldata(arr)
    ...
    return (
        <Box sx={{ display: 'flex', flex: 1, flexDirection: 'column', height: window.innerHeight - 100, overflowX: "auto" }}>
            <DasboardCustomLayout title={"Joining Letter"} displayClose={true} setClose={toRedirectToHome}>
                                            ...
                                            <Fragment key={val.desg_id}>
                                                ...
                                                {isRowOpen && matchingMainData && matchingMainData.apslno.map((item, idx) => {
                                                    ...
                                                    if (item?.status === 1) {
                                                        ...
                                                        return (
                                                            <TableRow key={idx} sx={item?.joining_status === 1 ? { backgroundColor: "#CDFADB" } :
                                                                item?.join_ltr_cancel_status === 1 ? { backgroundColor: "#FFACAC" } :
                                                                ...
                                                                {item?.joining_status === 1 || item?.join_ltr_cancel_status === 1 ?
                                                                ...
                                                                {item?.joining_status === 1 ?
                                                                    <TableCell size='small' sx={{ cursor: 'pointer' }} >
                                                                        Letter Downloaded
                                                                    </TableCell> :
                                                                    item?.join_ltr_cancel_status === 1 ?
                                                                        <TableCell size='small' sx={{ cursor: 'pointer' }} >
                                                                            Download Letter Canceled
                                                                        </TableCell> :
                                                                        <TableCell size='small' sx={{ cursor: 'pointer' }} >
                                                                            Download Pending
                                                                        </TableCell>
