...
const HODTrainingList = ({ em_department, em_dept_section }) => {
    ...
    useEffect(() => {
        ...
        const getDeptData = (async () => {
            const results = await axioslogin.post('/TrainingDashboard/HodInchargedeptmonthlytraining', obj)
            const { success, data } = results.data
            if (success === 1) {
                SetDeptdata(data)
            }
        ...
        const getDeptAllDetails = (async () => {
            const results = await axioslogin.post('/TrainingDashboard/HodInchargedeptmonthlyall', obj)
            const { success, data } = results.data
            if (success === 1) {
                SetDeptAll(data)
            }
    ...
    }, [month, em_department, em_dept_section])
    ...
    return (
        <Paper elevation={0} sx={{ flex: 1, boxShadow: 4, p: 1, backgroundColor: "#FFFFFF" }}>
            ...
            <Box sx={{ width: "100%" }}>
                <Box sx={{ width: "100%", height: 247, overflowY: "auto", mt: 1 }}>
...
export default memo(HODTrainingList) 
