...
const ApprovalPage = () => {
    ...
    useEffect(() => {
        ...
        const InductApprvls = async () => {
            const results = await axioslogin.post('/TrainingDashboard/InductAprvlcount', obj)
            ...
            if (success === 1) {
                ...
                const trainerApprvls = displayData.filter(val => val.trainer_induct_apprvl_status === 1 && val.training_status === 1 && val.posttest_status === 1);
                const trainerApprvlslen = trainerApprvls?.length
                SetInductTrainerApproved(trainerApprvlslen);
                const TrainerPending = displayData.filter(val => val.trainer_induct_apprvl_status === 0 && val.training_status === 1 && val.posttest_status === 1);
                const TrainerApprvlslen = TrainerPending?.length
                SetTrainerInductPending(TrainerApprvlslen);


                const HodApprvls = displayData.filter(val => val.training_induct_hod_aprvl_status === 1 && val.trainer_induct_apprvl_status === 1 && val.training_status === 1 && val.posttest_status === 1);
                const HodApprvlslen = HodApprvls?.length
                SetHodInductApproved(HodApprvlslen);
                const HodPending = displayData.filter(val => val.training_induct_hod_aprvl_status === 0 && val.trainer_induct_apprvl_status === 1 && val.training_status === 1 && val.posttest_status === 1);
        ...
        const DeptApprvls = async () => {
            const results = await axioslogin.post('/TrainingDashboard/DeptAprvlcount', obj)
            const { success, data } = results.data
            if (success === 1) {
                const displayData = data?.map((val) => {
                    const obj = {
                ...
                const trainerApprvls = displayData.filter(val => val.training_apprvl_status === 1 && val.training_status === 1 && val.posttest_status === 1);
                const trainerApprvlslen = trainerApprvls?.length
                SetDeptTrainerApproved(trainerApprvlslen);
                const TrainerPending = displayData.filter(val => val.training_apprvl_status === 0 && val.training_status === 1 && val.posttest_status === 1);
                const TrainerApprvlslen = TrainerPending?.length
                SetTrainerDeptPending(TrainerApprvlslen);


                const HodApprvls = displayData.filter(val => val.training_hod_apprvls_status === 1 && val.training_apprvl_status === 1 && val.training_status === 1 && val.posttest_status === 1);
                const HodApprvlslen = HodApprvls?.length
                SetHodDeptApproved(HodApprvlslen);
                const HodPending = displayData.filter(val => val.training_hod_apprvls_status === 0 && val.training_apprvl_status === 1 && val.training_status === 1 && val.posttest_status === 1);
                const HodPendinglen = HodPending?.length
        ...
        DeptApprvls()
    }, [SetInductTrainerApproved, SetTrainerInductPending, SetHodInductApproved, SetHodInductPending, SetDeptTrainerApproved, SetHodDeptApproved, SetHodDeptPending])
    return (
        <Fragment>

            <Paper elevation={0} sx={{ width: "100%", boxShadow: 4, p: 2, backgroundColor: "#FFFFFF" }}>
                <h5>Approvals</h5>
...
export default memo(ApprovalPage)
