...
const InductQREmpDetails = () => {
    ...
    const { retest_slno, em_name, re_questn_count,
        topic_slno, training_topic_name, dept_name, retest_em_no, em_no, desg_name, premark, postmark, training_status, retest_status, pretest, posttest, online, retest, retest_mark } = Viewdata;

    useEffect(() => {
        const getData = async () => {
            const result = await axioslogin.post('/TrainingEmployee_Dashboard/employeedeatils', postdata)
    ...
    useEffect(() => {
        if (Object.keys(tabledata).length !== 0) {
            const { sn, retest_slno, retest_em_no, re_emp_dept, re_dept_sec, em_name, date, re_topic, re_attendance, re_questn_count, em_id,
                sect_id, sect_name, topic_slno, training_topic_name, dept_id, dept_name, em_no, desg_name, desg_slno, premark, postmark, pretest_status, posttest_status, online_mode, training_status, retest_status, retest_mark } = tabledata[0];
            const obj = {
                sn: sn,
    ...
    const ClickToTest = useCallback(() => {
        if (training_status === 1 && retest_status === 0) {
            ...
            history.push(`/OnlineInductReTest/${retest_slno}/${retest_em_no}/${topic_slno}/${re_questn_count}`)
        }
        else if (training_status === 1 && retest_status === 1) {
            warningNofity("Already Attend the Retest Test")
        }

    }, [retest_slno, history, retest_em_no, topic_slno, re_questn_count, retest_status, training_status])
...
export default memo(InductQREmpDetails)
